...
import { StepPendingToApproved } from "./definition.ts";
import { DEFAULT_ALLOWED_USER_ID, runTransitionStep } from "../_shared/step_common.ts";

const ACTION_ID = "jira_step_pending_to_approved";

export default SlackFunction(StepPendingToApproved, async ({ inputs, client }) => {
  const blocks = [
    { type: "section", text: { type: "mrkdwn", text: `*Jira:* <${inputs.issueUrl}|${inputs.issueKey}>\nNext: *Pending Approval → Approved*` } },
    { type: "actions", elements: [{ type: "button", action_id: ACTION_ID, text: { type: "plain_text", text: "Pending Approval ➡ Approved" }, style: "primary", value: inputs.issueKey }] },
  ];

  const r = await client.chat.postMessage({ channel: inputs.channel_id, blocks, text: `Pending → Approved: ${inputs.issueKey}` });
  ...
  const result = await runTransitionStep({
    ...
    clicker,
    issueKey,
    transitionEnvKey: "JIRA_TRANSITION_PENDING_TO_APPROVED",
  });
  if (!result.ok) return;
