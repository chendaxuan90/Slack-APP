...
import { StepInProcessToInReview } from "./definition.ts";
import { DEFAULT_ALLOWED_USER_ID, runTransitionStep } from "../_shared/step_common.ts";

const ACTION_ID = "jira_step_in_process_to_in_review";

export default SlackFunction(StepInProcessToInReview, async ({ inputs, client }) => {
  const blocks = [
    { type: "section", text: { type: "mrkdwn", text: `*Jira:* <${inputs.issueUrl}|${inputs.issueKey}>\nNext: *In Process → In Review*` } },
    { type: "actions", elements: [{ type: "button", action_id: ACTION_ID, text: { type: "plain_text", text: "In Process ➡ In Review" }, style: "primary", value: inputs.issueKey }] },
  ];

  const r = await client.chat.postMessage({ channel: inputs.channel_id, blocks, text: `In Process → In Review: ${inputs.issueKey}` });
  ...
  const result = await runTransitionStep({
    ...
    clicker,
    issueKey,
    transitionEnvKey: "JIRA_TRANSITION_IN_PROCESS_TO_IN_REVIEW",
  });
  if (!result.ok) return;
