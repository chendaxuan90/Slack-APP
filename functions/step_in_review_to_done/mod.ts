...
import { StepInReviewToDone } from "./definition.ts";
import { DEFAULT_ALLOWED_USER_ID, runTransitionStep } from "../_shared/step_common.ts";

const ACTION_ID = "jira_step_in_review_to_done";

export default SlackFunction(StepInReviewToDone, async ({ inputs, client }) => {
  const blocks = [
    { type: "section", text: { type: "mrkdwn", text: `*Jira:* <${inputs.issueUrl}|${inputs.issueKey}>\nNext: *In Review → Done*` } },
    { type: "actions", elements: [{ type: "button", action_id: ACTION_ID, text: { type: "plain_text", text: "In Review ➡ Done" }, style: "primary", value: inputs.issueKey }] },
  ];

  const r = await client.chat.postMessage({ channel: inputs.channel_id, blocks, text: `In Review → Done: ${inputs.issueKey}` });
  ...
  const result = await runTransitionStep({
    ...
    clicker,
    issueKey,
    transitionEnvKey: "JIRA_TRANSITION_IN_REVIEW_TO_DONE",
  });
  if (!result.ok) return;
